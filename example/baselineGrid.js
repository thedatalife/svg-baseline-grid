...
(function () {
  ...
  function createAfterRule(baselineTopOffset, baselineGutters, encodedSvg) {
    return 'body:after {' + 'position: absolute;' + 'width: auto;' + 'height: auto;' + 'z-index: 9999;' + 'content: \'\';' + 'display: block;' + 'pointer-events: none;' + 'top: 0;' + 'right: 0;' + 'bottom: 0;' + 'left: 0;' + 'margin: ' + baselineTopOffset + 'px ' + baselineGutters + 'px 0 ' + baselineGutters + 'px;' + 'background: url(data:image/svg+xml;base64,' + encodedSvg + ')' + '}';
  }

  function createSVG(baselineHeight, strokeDash, strokeColor, strokeWidth) {
    var svg = '<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"  width="10px" height="' + baselineHeight + 'px" viewBox="0 0 10 ' + baselineHeight + '" enable-background="new 0 0 10 ' + baselineHeight + '" xml:space="preserve">' + '<line ' + 'x1="0" ' + 'y1="' + baselineHeight + '" ' + 'x2="10" ' + 'y2="' + baselineHeight + '" ' + 'stroke-dasharray="[' + strokeDash.join(',') + ']" ' + 'style="' + 'stroke:' + strokeColor + ';stroke-width: ' + strokeWidth + ';' + ' "/>' + '</svg>';
